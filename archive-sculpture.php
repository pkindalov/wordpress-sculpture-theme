...
<div class="sculpture-archive">
    ...
    <!-- Filters Component -->
    <?php 
    get_template_part('template-parts/sculpture/filters', null, array(
        'filters' => $filters,
...
function sculpture_build_query_args($filters) {
    ...
    if ($filters['year_min'] !== '' || $filters['year_max'] !== '') {
        $year_query = array('key' => 'year', 'type' => 'NUMERIC');
        
        if ($filters['year_min'] !== '' && $filters['year_max'] !== '') {
            $year_query['value'] = array($filters['year_min'], $filters['year_max']);
...
function sculpture_get_filter_data() {
    ...
    if ($sculptures->have_posts()) {
        while ($sculptures->have_posts()) {
            ...
            if ($price) {
                if ($data['min_price'] === 0) $data['min_price'] = $price;
