...
$promoted_label = sculpture_translate('Promoted', 'product_mode');
$featured_label = sculpture_translate('Featured', 'product_mode');
$ends_label = sculpture_translate('Ends', 'product_mode');
$no_image_label = sculpture_translate('no image available', 'common');
?>
...
<article class="sculpture-card <?php echo $is_featured ? 'is-featured' : ''; ?> <?php echo $is_on_promo ? 'is-on-promo' : ''; ?>">
    ...
    <?php if ($is_on_promo && $promo_pct): ?>
        <span class="card-badge badge-promo">-<?php echo esc_html($promo_pct); ?>%</span>
    <?php elseif ($is_on_promo): ?>
        <span class="card-badge badge-promo">
            <?php echo $promoted_label; ?>
        </span>
        ...
        <?php else: ?>
            <div class="card-image card-image-placeholder">
                <svg viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="200" y="330" font-family="Arial, sans-serif" font-size="14" font-weight="600" fill="#6A8B58" text-anchor="middle" letter-spacing="2"><?php echo mb_strtoupper($no_image_label); ?></text>
                </svg>
            </div>
        <?php endif; ?>
    </a>
    ...
    <div class="card-content">
        ...
        <?php if ($year || $materials): ?>
            <div class="card-meta">
                <span class="card-meta-item">
                    ...
                    echo $materials ? sculpture_translate($materials, 'material') : '';
                    ?>
                </span>
            </div>
        <?php endif; ?>
            ...
            <div class="card-footer">
                ...
                <div class="card-price-wrapper">
                    <?php if ($is_on_promo && $promo_price): ?>
                        <span class="card-price-original">
                            <?php 
                            echo esc_html(number_format($price, 0) . $currency . ' ');
                            sculpture_display_price_in_leva($price);
                            ?>
                        </span>
                        <span class="card-price card-price-promo">
                            <?php 
                            echo esc_html(number_format($promo_price, 0) . $currency . ' ');
                            sculpture_display_price_in_leva($promo_price);
                            ?>
                        </span>
                    <?php elseif ($price): ?>
                ...
                <?php if ($availability): ?>
                    <span class="card-status status-<?php echo esc_attr(sanitize_title($availability)); ?>">
                        <?php echo sculpture_translate($availability, 'availability'); ?>
                    </span>
                <?php endif; ?>
        ...
        <?php if ($is_on_promo && $promo_ends): ?>
            <div class="card-promo-ends">
                ...
                <?php echo $ends_label; ?>: <?php echo sculpture_translate_date(date('d M Y', strtotime($promo_ends))); ?>
            </div>
        <?php endif; ?>
